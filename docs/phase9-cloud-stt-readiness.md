...
状态：腾讯云 STT、家庭节点回退、免费 Cloud TTS、生产部署和自动化验收已完成；iPhone 已通过真实麦克风识别与回复播放主闭环，播放中断和强制故障文字降级待验收。
...
- 生产日志确认 iPhone 真机语音由腾讯 Provider 正确识别，用户确认识别文字正确。
- iOS 客户端 `0.3.2+5` 显式声明 `audio/mpeg` 并恢复播放型音频会话；Codemagic 构建 68 成功，用户确认真机可正常听到回复。
- 已下载 IPA 的应用内版本为 `0.3.2 (5)`，SHA-256 为 `0BEB3124EFFF6D3F6E5D315A4EDA3722C8544795813B8ECC09A3890145BCEB8D`。
...
1. 验证 TTS 播放中断与服务不可用时的文字降级。
2. 关闭 Phase 8 其余物理验收项。
3. 推进 Phase 10 完整备份、恢复与监控。
