...
[![Status](https://img.shields.io/badge/status-maintenance-lightgrey.svg)]()
...
日报推送目前已暂停日常使用，代码、配置、历史数据和聊天分析能力继续保留。手机助手后端可选复用这里的 `miru.chat_analyzer`，本项目仍可独立安装和运行。
...
cd miru-assistant/products/daily-report
...
> 📖 **完整使用指南**：详见 [MEDIA_EXPORT_GUIDE.md](docs/guides/MEDIA_EXPORT_GUIDE.md)
...
├── scripts/                        # 运维脚本
├── docs/                           # 架构、指南、运维与发布历史
├── data/                           # 数据库、模型、缓存与日志（gitignore）
├── output/                         # 聊天导出与分析结果（gitignore）
├── backup/、chatlog/               # 本机备份和聊天工具数据（gitignore）
└── tools/                          # 第三方微信数据工具（gitignore）
```
...
> 当前为维护模式；新增产品功能优先放在 `products/mobile-assistant/`。
